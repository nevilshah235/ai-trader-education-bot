 ...
 * When true, use oauth.deriv.com + session tokens in redirect + public WebSocket.
 */
...
export const getSocketURL = async (): Promise<string> => {
    try {
        ...
        if (isDerivThirdPartyAuth()) {
            const token = typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('deriv_oauth_token') : null;
            if (token) {
...
export const generateOAuthURL = async () => {
    try {
        ...
        // Use brand config for login URLs (dbot/internal auth)
